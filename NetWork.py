...
class ResNet(nn.Module):
    def __init__(self,
        ...
        ### YOUR CODE HERE
        # define conv1
        self.start_layer = nn.Conv2d(in_channels=3,out_channels=self.first_num_filters,kernel_size=7,stride=2,padding=3, bias=False)
...
class standard_block(nn.Module):
    ...
    def __init__(self, filters, projection_shortcut, strides, first_num_filters) -> None:
        ...
        elif filters == first_num_filters:
            in_filters = filters
        else:
        ...
        ### YOUR CODE HERE
        ### YOUR CODE HERE
        self.activation1 = batch_norm_relu_layer(first_num_filters,eps=1e-5,momentum=0.997)
        self.conv1 = nn.Conv2d(in_channels=first_num_filters,out_channels=filters, kernel_size=3,stride=strides,padding=1)
        self.batchnorm = nn.BatchNorm2d(num_features=filters,eps=1e-5,momentum=0.997)
        self.relu = nn.ReLU()
        self.conv2 = nn.Conv2d(in_channels=filters,out_channels=filters,kernel_size=3,stride= strides,padding=1)
        ### YOUR CODE HERE
...
class bottleneck_block(nn.Module):
    ...
    def __init__(self, filters, projection_shortcut, strides, first_num_filters) -> None:
        ...
        self.activation1 = batch_norm_relu_layer(in_filters,eps=1e-5,momentum=0.997)
        self.conv1 = nn.Conv2d(in_channels=in_filters,out_channels=filters,kernel_size=1,stride=strides)

        self.activation2 = batch_norm_relu_layer(filters,eps=1e-5,momentum=0.997)
        self.conv2 = nn.Conv2d(in_channels=filters,out_channels=filters,kernel_size=3,padding=1,stride=strides)

        self.activation3 = batch_norm_relu_layer(filters,eps=1e-5,momentum=0.997)
        self.conv3 = nn.Conv2d(in_channels=filters,out_channels=filters*4,kernel_size=1,stride=strides)
    ...
    def forward(self, inputs: Tensor) -> Tensor:
        ...
        identity = inputs
        out = self.activation1(inputs)
        if self.projection_shortcut is not None:
            identity = self.projection_shortcut(out)
...
class stack_layer(nn.Module):
    ...
    def __init__(self, filters, block_fn, strides, resnet_size, first_num_filters) -> None:
        ...
        ### END CODE HERE
        nr_channels = filters_out //2 if filters != first_num_filters else filters
        projection_shortcut = nn.Conv2d(nr_channels,filters_out,kernel_size=1,stride=strides,padding=0)
        self.layers = nn.ModuleList()
        self.layers.append(block_fn(filters, projection_shortcut, strides,first_num_filters))

        for i in range(1,resnet_size):
            self.layers.append(block_fn(filters,None,1,first_num_filters))

        """if strides != 1:
            projection_shortcut = nn.Sequential(
                nn.Conv2d(first_num_filters,filters_out,kernel_size=1, stride=strides),
                nn.BatchNorm2d(filters_out))
        # Only the first block per stack_layer uses projection_shortcut and strides

        layers = []
        layers.append(block_fn(filters_out, projection_shortcut,strides,first_num_filters))
        first_num_filters = first_num_filters * block_fn.expansion
        for i in range(1,resnet_size):
            layers.append(block_fn(filters_out,first_num_filters))"""
